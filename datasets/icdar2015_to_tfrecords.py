...
def cvt_to_tfrecords(output_path , data_path, gt_path):
    ...
    with tf.python_io.TFRecordWriter(output_path) as tfrecord_writer:
        for idx, image_name in enumerate(image_names):
            ...
            example = convert_to_example(image_data, str.encode(image_name), labels, labels_text, bboxes, oriented_bboxes, shape)
            tfrecord_writer.write(example.SerializeToString())
        
if __name__ == "__main__":
    '''
    root_dir = util.io.get_absolute_path('~/tmp/')
    ...
    root_dir = util.io.get_absolute_path('~/tmp/')
    output_dir = util.io.get_absolute_path('~/dataset/pixel_link/msra/')
    util.io.mkdir(output_dir);

    training_data_dir = util.io.join_path(root_dir, 'msra_training_images')
    training_gt_dir = util.io.join_path(root_dir,'msra_training_location_gt')
    cvt_to_tfrecords(output_path = util.io.join_path(output_dir, 'msra_train.tfrecord'), data_path = training_data_dir, gt_path = training_gt_dir)
